...
public class Shell {
	...
	public static List<String> run(String shell, String[] commands,
		...
		try {
			...
			try {
				for (String write : commands) {
					Debug.logCommand(String.format("[%s+] %s", shellUpper,
							write));
					STDIN.write((write + "\n").getBytes("UTF-8"));
					STDIN.flush();
				}
				...
				if (e.getMessage().contains("EPIPE")) {
					...
					// STDIN, the script already contained the exit command,
					// etc.
		...
		Debug.logCommand(String.format("[%s%%] END",
				shell.toUpperCase(Locale.ENGLISH)));
		return res;
	}
	 ...
	 */
	protected static boolean parseAvailableResult(List<String> ret,
	...
	public static class SU {
		...
		public static synchronized String version(boolean internal) {
			...
			if (suVersion[idx] == null) {
				...
				List<String> ret = Shell.run(internal ? "su -V" : "su -v",
						new String[] { "exit" }, null, false);
		...
		public static String shell(int uid, String context) {
			...
			if ((context != null) && isSELinuxEnforcing()) {
				...
				if ((display != null) && (internal != null)
						&& (display.endsWith("SUPERSU"))
						&& (Integer.valueOf(internal) >= 190)) {
					shell = String.format(Locale.ENGLISH, "%s --context %s",
							shell, context);
				}
			}
	...
	public static class Builder {
		 ...
		 * 
		 * @param useMinimal
		 *            true for reduced output, false for full output
		 * @return This Builder object for method chaining
		 */
		public Builder setMinimalLogging(boolean useMinimal) {
			Debug.setLogTypeEnabled(Debug.LOG_COMMAND | Debug.LOG_OUTPUT,
					!useMinimal);
			return this;
		}
	...
	public static class Interactive {
		...
		private Interactive(final Builder builder,
			...
			watchdogTimeout = 60;
			addCommand(Shell.availableTestCommands, 0,
					new OnCommandResultListener() {
						public void onCommandResult(int commandCode,
								int exitCode, List<String> output) {
							if (exitCode == OnCommandResultListener.SHELL_RUNNING
									&& Shell.parseAvailableResult(output,
											Shell.SU.isSU(shell)) != true) {
		...
		public synchronized void addCommand(String[] commands, int code,
				OnCommandLineListener onCommandLineListener) {
			this.commands.add(new Command(commands, code, null,
					onCommandLineListener));
			runNextCommand();
		}
		...
		private void runNextCommand(boolean notifyIdle) {
			...
			if (running && idle && (commands.size() > 0)) {
				...
				while (commands.size() > 0) {
					postCallback(commands.remove(0),
							OnCommandResultListener.SHELL_DIED, null);
				}
			}
		...
		private void postCallback(final Command fCommand, final int fExitCode,
			...
			if (handler == null) {
				...
				if (fCommand.onCommandLineListener != null)
					fCommand.onCommandLineListener.onCommandResult(
		...
		private synchronized boolean open() {
			Debug.log(String.format("[%s%%] START",
					shell.toUpperCase(Locale.ENGLISH)));
		...
		public void close() {
			...
			// throw in debug mode.
			if (!_idle && Debug.getSanityChecksEnabledEffective()
					&& Debug.onMainThread()) {
